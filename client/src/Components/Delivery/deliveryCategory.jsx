import React from "react";


const DeliverySmCard=({image,title})=>{
    return <> 
    <div className="lg:hidden bg-white shadow md:w-56 rounded-md">
        <div className="w-full h-24  ">
            <img  src={image}  alt="food" className="w-full h-full object-cover rounded-t-lg" />
        </div>
        <div>
            <h3 className="text-sm my-1 text-center font-light">{title}</h3>
        </div>
    </div>
</>

}

const DeliveryLgCard=({image,title})=>{
    return <> 
    <div className="hidden lg:block w-64 h-48">
        <div className="w-full h-full  ">
            <img  src={image}  alt="food" className="w-full h-full object-cover shadow-lg rounded" />
        </div>
        <div>
            <h3 className="text-xl my-1  font-medium">{title}</h3>
        </div>
    </div>
</>

}


const DeliveryCategory = (props)=>{
    return <>
    {/* for small and medium devices */}
        <DeliverySmCard {...props}/>

        {/* for large devices */}
        <DeliveryLgCard {...props}/>
    </>
}

export default DeliveryCategory;